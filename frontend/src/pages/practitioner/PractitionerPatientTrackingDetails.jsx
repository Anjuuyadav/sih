...
import { completeTrackingSession, getTrackingPatientDetails } from '../../services/practitionerService';
...
const PractitionerPatientTrackingDetails = () => {
  ...
  const loadTracking = useCallback(async () => {
    ...
    setError('');
    try {
      setTracking(await getTrackingPatientDetails(patientId));
    } catch (requestError) {
      setError(requestError.message || 'Unable to load patient therapy tracking.');
    } finally {
      setLoading(false);
    }
  }, [patientId]);
  ...
  const markCompleted = async (sessionId) => {
    ...
    setMessage('');
    try {
      await completeTrackingSession(sessionId);
      setMessage('Session marked as completed.');
      await loadTracking();
    } catch (requestError) {
      setError(requestError.message || 'Unable to mark session completed.');
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-white px-4 py-6 sm:px-8">
      <div className="mx-auto max-w-6xl">
        <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            <Link to="/practitioner/therapy-tracking" className="text-sm font-semibold text-primary-600">
              &larr; Back to patients
            </Link>
            <h1 className="mt-2 text-3xl font-semibold text-slate-900">Patient Therapy Tracking</h1>
            <p className="mt-1 text-slate-500">Track progress and mark completed sessions.</p>
          </div>
        ...
        {error && <div role="alert" className="mb-4 rounded-2xl border border-error-500/20 bg-error-50 px-4 py-3 text-sm text-error-600">{error}</div>}
        {message && <div role="status" className="mb-4 rounded-2xl border border-success-500/20 bg-success-50 px-4 py-3 text-sm text-success-600">{message}</div>}
        ...
        {loading ? (
          ...
          <>
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              <h2 className="text-xl font-semibold text-slate-900">Patient: {tracking.patient?.name || 'Patient'}</h2>
              <p className="mt-1 text-sm text-slate-500">Overall progress across accepted therapy plans</p>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-4">
                ...
                <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs text-slate-400">Progress</p><p className="mt-1 text-2xl font-semibold text-primary-700">{tracking.progressPercentage}%</p></div>
              </div>
                ...
                <article key={plan.therapyPlanId} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">Treatment plan #{plan.therapyPlanId}</p>
                    </div>
                  ...
                  <h4 className="mt-5 flex items-center gap-2 text-lg font-semibold text-slate-900"><FiClock className="text-primary-600" /> Scheduled Sessions</h4>
                  {plan.sessions.length ? (
                      ...
                      <table className="w-full min-w-[760px] text-left text-sm">
                        ...
                        <tbody>
                          {plan.sessions.map((session) => {
                            ...
                            const inFlight = updatingSessionId === session.sessionId;
                            return (
                              <tr key={session.sessionId} className="border-t border-slate-100">
                                <td className="py-3 font-semibold text-slate-700">{session.sessionNumber}</td>
                                <td className="py-3 text-slate-600">{formatDate(session.sessionDate)}</td>
                                <td className="py-3 text-slate-600">{formatTime(session.startTime)}</td>
                                <td className="py-3 text-slate-600">{formatTime(session.endTime)}</td>
                                <td className="py-3">
                                  {isCompleted ? (
                                    <span className="inline-flex items-center gap-1 font-semibold text-success-600"><FiCheckCircle /> Completed</span>
                                  ) : (
                                ...
                                <td className="py-3">
                                  {canComplete ? (
                                    <button type="button" disabled={Boolean(updatingSessionId)} onClick={() => markCompleted(session.sessionId)} className="rounded-lg bg-success-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-success-500 disabled:cursor-not-allowed disabled:opacity-60">
                                      {inFlight ? 'Updating...' : 'Mark Completed'}
                                    </button>
                                  ) : (
                    ...
                    <div className="mt-3 rounded-2xl border border-dashed border-slate-300 p-6 text-sm text-slate-500">No scheduled sessions found.</div>
                  )}
                </article>
              ))}
            </div>
...
export default PractitionerPatientTrackingDetails;
